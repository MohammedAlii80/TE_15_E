...
class TestPickingBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_delivery_lot_with_multi_companies(self):
        ...
        sn_a_1 = self.env['stock.production.lot'].create({'name': 'tsn-001', 'product_id': self.productserial1.id, 'company_id': company_a.id})
        sn_a_2 = self.env['stock.production.lot'].create({'name': 'tsn-002', 'product_id': self.productserial1.id, 'company_id': company_a.id})
        sn_b_1 = self.env['stock.production.lot'].create({'name': 'tsn-001', 'product_id': self.productserial1.id, 'company_id': company_b.id})
        sn_b_2 = self.env['stock.production.lot'].create({'name': 'tsn-003', 'product_id': self.productserial1.id, 'company_id': company_b.id})
    ...
    def test_delivery_lot_with_package_delivery_step(self):
        ...
        sn = self.env['stock.production.lot'].create({'name': 'sn', 'product_id': self.productlot1.id, 'company_id': self.env.company.id})
    ...
    def test_nomenclature_alias_and_conversion(self):
        """ Ensures that if an alias is scanned, then the Barcode App uses it correctly.
        Also, checks when an EAN-13 is scanned, if the conversion to an UPC-A should be done,
        the conversion is correctly handled and the Barcode App actually uses the converted barcode.
        """
        clean_access_rights(self.env)
        nomenclature = self.env.ref('barcodes.default_barcode_nomenclature')
        nomenclature.upc_ean_conv = 'always'
        self.env.company.nomenclature_id = nomenclature
        # Creates a product using a UPC-A as a barcode.
        self.env['product.product'].create({
            'name': 'Product Test, UPC-A barcode',
            'type': 'product',
            'categ_id': self.env.ref('product.product_category_all').id,
            'barcode': '123123123125',
        })
        # Creates some rules for the nomenclature. The sequence order is important
        # since the parser will try every rules following the sequence order.
        self.env['barcode.rule'].create([
            {
                'name': "Alias for Product Test (UPC-A)",
                'sequence': 1,
                'type': 'alias',
                'pattern': 'alias_for_upca',
                'alias': '123123123125',
                'barcode_nomenclature_id': nomenclature.id,
            }, {
                'name': "Alias for Product Test (EAN-13)",
                'sequence': 2,
                'type': 'alias',
                'pattern': 'alias_for_ean13',
                'alias': '0123123123125',
                'barcode_nomenclature_id': nomenclature.id,
            }, {
                'name': "Convert EAN-13 to UPC-A",
                'sequence': 3,
                'type': 'product',
                # The pattern should math the UPC-A, not the EAN-13.
                'pattern': r'\d\d\d\d\d\d\d\d\d\d\d\d',  # Means 12 digits.
                'encoding': 'upca',
                'barcode_nomenclature_id': nomenclature.id,
            },
        ])
        ...
        action_id = self.env.ref('stock_barcode.stock_barcode_action_main_menu')
        url = "/web#action=" + str(action_id.id)
        self.start_tour(url, 'test_nomenclature_alias_and_conversion', login='admin', timeout=180)
    ...
    def test_gs1_delivery_ambiguous_serial_number(self):
        ...
        product_a, product_b = self.env['product.product'].create([{
            ...
            'barcode': barcode,
            'tracking': 'serial',
        } for i, barcode in enumerate(['05711544001952', '05711544001953'])])
        # Creates 2 serial numbers (same name different product).
        lot_b, lot_a = self.env['stock.production.lot'].create([
...
class TestInventoryAdjustmentBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_gs1_inventory_lot_serial(self):
        ...
        # Without demo data -> Cannot read properties of undefined (reading 'rainbowMan')
        if not loaded_demo_data(self.env):
            _logger.warning("This test relies on demo data. To be rewritten independently of demo data for accurate and reliable results.")
            return
    ...
    def test_scan_barcode_two_locations_multi_companies(self):
        clean_access_rights(self.env)
        grp_pack = self.env.ref('stock.group_tracking_lot')
        grp_multi_loc = self.env.ref('stock.group_stock_multi_locations')
        self.env.user.write({'groups_id': [(4, grp_multi_loc.id, 0), (4, grp_pack.id, 0)]})
        la_company = self.env.company
        company_a = self.env['res.company'].create({'name': 'copma LLC'})
        self.env["stock.location"].create([
            {
                'name': 'locationA',
                'company_id': la_company.id,
                'barcode': '1234567',
                'location_id': self.stock_location.id,
            },
            {
                ...
                'company_id': company_a.id,
                'barcode': '1234567',
                'location_id': self.env['stock.warehouse'].search([('company_id', '=', company_a.id)]).lot_stock_id.id
            }
        ])
        self.env.user.write({
            'company_ids': [(4, company_a.id), (4, la_company.id)],
            'company_id': company_a.id,
        })
        ...
        action_id = self.env.ref('stock_barcode.stock_barcode_action_main_menu')
        url = "/web#action=" + str(action_id.id)
        self.start_tour(url, 'test_same_barcode_multi_company', login='admin', timeout=180)
